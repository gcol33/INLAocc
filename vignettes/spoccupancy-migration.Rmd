---
title: "Migrating from spOccupancy"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Migrating from spOccupancy}
...
temporal, and integrated designs, all fitted via Polya-Gamma MCMC. If you
have used spOccupancy for any of these model types, you already know the
data format and the modelling workflow that INLAocc builds on.
...
sampling entirely. The user-facing consequence is speed: models that take
minutes under MCMC finish in seconds under INLA, with no burn-in, no
thinning, and no convergence diagnostics. The statistical consequence is
that posteriors are approximate rather than exact --- but for
...
honest caveats about when MCMC remains the better choice. For model-specific
details, see `vignette("spatial-models")`, `vignette("multi-species")`, and
...
**Unified interface.** One function (`occu()`) handles all 17 model types
via keyword arguments. No need to remember whether the spatial
multi-species version is `spMsPGOcc()` or `stMsPGOcc()`.

**When NOT to switch.** Three situations favour staying with spOccupancy:

1. You need exact posterior samples for custom derived quantities that go
   beyond what `predict()` provides. INLAocc generates posterior samples
   via INLA's Gaussian approximation, which is accurate for marginals but
   does not capture the full joint posterior.
2. You need informative inverse-Gamma priors on specific variance
   components. INLAocc uses PC priors (penalizing complexity), which are
   conceptually different and not easily mapped to inverse-Gamma
   hyperparameters.
...
**Structured data objects.** INLAocc also provides `occu_data()` and
`occu_format()` for constructing validated data objects with built-in
checks. These are optional --- raw spOccupancy lists work without
conversion. But if you are starting a new analysis (rather than migrating
an existing one), the structured constructors catch formatting errors
earlier.


## Function mapping

Every spOccupancy model function maps to a single `occu()` call with
different keyword arguments. The table below covers all 17 model types:

| spOccupancy function | INLAocc equivalent | Notes |
|:---|:---|:---|
| `PGOcc()` | `occu(~ occ, ~ det, data)` | Basic single-species |
| `spPGOcc()` | `occu(..., spatial = coords)` | Spatial (SPDE mesh) |
| `tPGOcc()` | `occu(..., temporal = "ar1")` | Multi-season |
| `stPGOcc()` | `occu(..., spatial = coords, temporal = "ar1")` | Spatio-temporal |
| `svcPGOcc()` | `occu(..., spatial = coords, svc = k)` | Spatially varying coefficients |
| `svcTPGOcc()` | `occu(..., spatial = coords, svc = k, temporal = "ar1")` | SVC + temporal |
| `msPGOcc()` | `occu(..., multispecies = TRUE)` | Community (multi-species) |
| `spMsPGOcc()` | `occu(..., multispecies = TRUE, spatial = coords)` | Spatial community |
| `tMsPGOcc()` | `occu(..., multispecies = TRUE, temporal = "ar1")` | Temporal community |
| `stMsPGOcc()` | `occu(..., multispecies = TRUE, spatial = coords, temporal = "ar1")` | Spatio-temporal community |
| `svcTMsPGOcc()` | `occu(..., multispecies = TRUE, spatial = coords, svc = k, temporal = "ar1")` | MS + SVC + temporal |
...
| `lfJSDM()` | `occu(~ occ, data, multispecies = "jsdm", n.factors = k)` | Joint species distribution |
| `sfJSDM()` | `occu(~ occ, data, multispecies = "jsdm", n.factors = k, spatial = coords)` | Spatial JSDM |
| `intPGOcc()` | `occu(..., integrated = TRUE)` | Integrated data sources |
| `spIntPGOcc()` | `occu(..., spatial = coords, integrated = TRUE)` | Spatial integrated |

The pattern is consistent: `spatial = coords` adds a spatial field,
`temporal = "ar1"` adds temporal structure, `multispecies = TRUE` enables
community modelling, and `n.factors = k` activates latent factor
dimension reduction.
...
packages. We use `spOccupancy::simOcc()` to generate data with known
parameters, then compare the code and output.

### spOccupancy version

```{r spocc-example, eval = FALSE}
library(spOccupancy)
...
with `+`, `:`, `*`, `I()`, `poly()`, and `ns()` all working as
...
Samples are generated on first access via `INLA::inla.posterior.sample()`
and cached in the fit object. This means downstream code that operates on
spOccupancy posterior matrices --- computing derived quantities, feeding
into custom plotting functions, or passing to other packages --- works
with INLAocc fits without modification.

However, INLAocc also provides native S3 methods that are often more
convenient than working with raw sample matrices:

```{r s3-methods, eval = FALSE}
...
- **Traceplots** --- these diagnose chain mixing. No chains, no mixing
  to diagnose.
...
whether the fitted model captures the data-generating process --- which
is orthogonal to whether the estimation algorithm converged. See
`vignette("diagnostics")` for worked examples.
...
decision space eliminates a common source of analytical variability.
Two researchers fitting the same model to the same data with different
MCMC settings can get different posteriors; two researchers using INLAocc
cannot.

**Deterministic convergence.** Run the same `occu()` call twice and get
identical results. This simplifies debugging, makes analyses
reproducible without seed management, and removes "did my chains
converge?" from the review checklist.
...
include. These complement posterior predictive checks by testing specific
aspects of model fit rather than a single omnibus statistic.
...
- **Heavy posterior skewness.** If the scientific question depends on
  the tail shape of a posterior (e.g., probability that an effect exceeds
  a threshold), MCMC's exact samples may be more reliable than INLA's
...
arises from treating the EM point estimates of latent occupancy states as
fixed during INLA fitting. The correction is fast (negligible runtime
...
number of groups is small (< 10), full MCMC via spOccupancy is the safer
choice.
...
For moderate-to-large datasets, the prior choice has negligible impact on
posteriors --- the likelihood dominates. For small datasets, priors
...
spOccupancy gives you full MCMC samples from the joint posterior. You can
compute any derived quantity (species richness at a site, probability of
co-occurrence, etc.) by applying a function to each posterior draw.
...
joint posterior sampling. For simple derived quantities (posterior means,
credible intervals, posterior probabilities of direction), the
...
1. **Install INLAocc and INLA.** INLAocc requires the INLA package, which
   is not on CRAN. See `vignette("quickstart")` for installation
   instructions.
...
3. **Replace the model call.** Swap the spOccupancy function and its
   MCMC arguments for a single `occu()` call:
   ```{r migration-call, eval = FALSE}
   # Before
...
5. **Replace diagnostic functions.** `ppcOcc()` becomes `ppcOccu()`,
   `waicOcc()` becomes `waicOccu()`.
